...
function Wait-NewDesktopModel {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        foreach ($candidate in $allCandidates) {
            ...
            if ($null -ne $candidate.ProcessId) { [void]$observedProcessIds.Add([int]$candidate.ProcessId) }

            $identity = Get-CandidateIdentity -Candidate $candidate
            $isNewIdentity = -not $PreviousCandidates.ContainsKey($identity)
            $isNewProcess = ($null -ne $candidate.ProcessStartUtc -and $candidate.ProcessStartUtc -ge $StartedUtc.AddSeconds(-5))
            $isRecentWorkspace = ($candidate.LastWriteTimeUtc -gt [DateTime]::MinValue -and $candidate.LastWriteTimeUtc -ge $StartedUtc.AddSeconds(-5))
        ...
        $signature = (@($current | ForEach-Object { "$($_.Port):$($_.ProcessId):$($_.Source)" }) -join ',')
        if ($signature -ne $lastCandidateSignature) {
            $lastCandidateSignature = $signature
            if ($current.Count -gt 0) {
                Write-ProgressEvent -Phase 'model-candidates' -Message ("Detected $($current.Count) candidate local model endpoint(s): " + $signature)
            }
        }
        ...
        foreach ($candidate in $current) {
            ...
            if ($info.TableCount -gt 0 -and $null -ne $previousCount -and [int]$previousCount -eq $info.TableCount) {
                return [PSCustomObject]@{
    ...
    $portsText = if ($observedPorts.Count -gt 0) { (@($observedPorts) | Sort-Object) -join ', ' } else { 'none' }
    $processesText = if ($observedProcessIds.Count -gt 0) { (@($observedProcessIds) | Sort-Object) -join ', ' } else { 'none' }

    throw @"
Power BI Desktop opened the PBIX, but pbi-profiling could not correlate a new
stable local Analysis Services model before the timeout. No manual Save As is
required. Discovery observed ports: $portsText; msmdsrv process IDs:
$processesText. The adapter checks both live msmdsrv TCP listeners and
msmdsrv.port.txt workspace files. If Power BI Desktop is showing a credential,
privacy-level, refresh, recovery, or other blocking dialog, resolve that dialog
and retry. Otherwise report this exact message so the remaining identity issue
...
foreach ($candidate in Get-DesktopModelCandidates) {
    $beforeCandidates[(Get-CandidateIdentity -Candidate $candidate)] = $true
}
...
function Get-CandidateIdentity {
    ...
    $processId = if ($null -ne $Candidate.ProcessId) { [string]$Candidate.ProcessId } else { '' }
    $workspace = if ($Candidate.Workspace) { [string]$Candidate.Workspace } else { '' }
    return "$($Candidate.Port)|$processId|$workspace"
}
