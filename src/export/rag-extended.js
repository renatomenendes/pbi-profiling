  ...
  if (sourceResolution) {
    chunks.push({
      ...
      text: [
        ...
        `External resource-lineage coverage: ${formatPercent(sourceResolution.summary?.resourceLineageCoverage)}.`,
        `External physical-column coverage: ${formatPercent(sourceResolution.summary?.physicalColumnCoverage)}.`,
        `External columns resolved to a physical column: ${sourceResolution.summary?.physicalColumnResolved ?? 0}.`,
        `External columns resolved only to a physical resource: ${sourceResolution.summary?.resourceResolved ?? 0}.`,
        `Inline/model-local columns: ${sourceResolution.summary?.inlineColumns ?? 0}.`,
        `Columns still unresolved: ${sourceResolution.summary?.unresolvedColumns ?? 0}.`,
        `Computed columns excluded from external source-lineage denominator: ${sourceResolution.summary?.computedColumns ?? 0}.`,
        sourceResolution.methodology?.inlinePolicy,
